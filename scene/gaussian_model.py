...
class GaussianModel:
    ...
    def get_features(self):
        ...
        # 当 SH degree=0 时，_features_rest 为空，直接返回 DC
    ...
    def gaussian_init(self):
        # 随机生成服从高斯分布的张量
        init_points = 20000  # 减少点数以加速验证
        fused_point_cloud = torch.randn((init_points,3)).float().cuda()
        fused_point_cloud = (fused_point_cloud*20)
        fused_color = RGB2SH(torch.rand((init_points,3)).float().cuda())
    ...
    def create_from_map(self, map_points, n_init=20000, spatial_lr_scale=50.0,
        ...
        # 初始化旋转 (单位四元数)
        rots = torch.zeros((n_init, 4), device="cuda")
        rots[:, 0] = 1

        # 初始化不透明度
        opacities = self.inverse_opacity_activation(
            0.1 * torch.ones((n_init, 1), dtype=torch.float, device="cuda")
        )
